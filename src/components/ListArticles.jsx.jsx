    ...
    return (
        <div className='space-y-6'>
            ...
            <div className='bg-gray-100 flex justify-end sm:justify-between items-center py-7 px-4 sm:px-6 font-poppins text-gray-800'>
                <div className='hidden sm:block'>Showing {itemsOnCurrPage.length} of {articles?.length} items</div>
                <div className='block lg:hidden'>
                    <button className='flex items-center text-sm px-3 sm:px-5 md:text-base py-3 gap-1 sm:gap-4 rounded-full bg-gray-600 sm:font-light text-white  ' onClick={toggleFilter}>
                    ...
                    itemsOnCurrPage?.length > 0 ?
                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                            {itemsOnCurrPage?.map((item) => (
                                <div key={item.id} className='h-[450px] shadow-gray-400 shadow-md border-black overflow-hidden rounded-sm bg-white  lg:hover:scale-105 transition-transform duration-300 '>
                                    <Card
                                        title={item.title}
                                        id={item.id}
            ...
            <div>
                <div className=' bg-gray-100 flex flex-col gap-3 lg:flex-row justify-between items-center py-5 px-6 font-poppins text-gray-800 text-sm text-normal sm:text-base'>
                    <div>Showing {currentPage}-{totalPages} of {articles?.length} items</div>
                    <div className='flex items-center gap-2'>
                        <button onClick={prevPage} className="p-4 bg-white rounded-sm">{"Prev"}</button>
                        <button onClick={nextPage} className="p-4 bg-white rounded-sm">{"Next"}</button>
                    </div>
                </div>
            </div>
